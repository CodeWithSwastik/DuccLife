...
class Upgrades(commands.Cog):
    ...
    async def upgrades(self, ctx):
        ...
        embed = discord.Embed(
            title="Upgrades",
            description=f"Use {ctx.prefix}upgrade `fuel|engine|velocity` to upgrade your rocket.",
            color=discord.Color.random(),
        )
        embed.add_field(
            name="Engine",
            value=f"+5 Peta Newton\nCost: {calc_cost(data.thrust, *self.price_map['engine'])} {self.bot.duckcoin}",
            inline=False,
        )
        embed.add_field(
            name="Fuel Tank",
            value=f"+1 Zeta liters\nCost: {calc_cost(data.fuel, *self.price_map['fuel'])} {self.bot.duckcoin}",
        )
        embed.add_field(
            name="Max Velocity",
        ...
        embed.set_thumbnail(
            url="https://media.discordapp.net/attachments/682157382062964781/919485727585210388/duck-rocket1.png"
        )
    ...
    async def upgrade(self, ctx, part: Literal["fuel", "velocity", "engine"]):
        ...
        if cost > userdata.money:
            return await ctx.send(
                f"You don't have {cost} {self.bot.duckcoin} to do this upgrade!"
            )

        await self.bot.db.update_money(ctx.author, -cost)
        await self.bot.db.update_rocket(ctx.author, part, self.price_map[part][-1])
        await ctx.send(
            f"Your rocket has been upgraded! Use `{ctx.prefix}stats` to see the new stats!"
        )
    ...
    async def train(self, ctx):
        ...
        if 10 > userdata.money:
            return await ctx.reply(
                f"You don't have 10 {self.bot.duckcoin} to buy equipment for this training session!"
            )
        ...
        if random.randrange(1, 11) > 8:
            ...
            await ctx.reply(
                f"The training wasn't successful! Your duck seems be to be in a lot of stress."
            )
        else:
