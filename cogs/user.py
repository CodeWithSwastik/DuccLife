...
class User(commands.Cog):
    ...
    async def profile(self, ctx, user: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"{user.name}'s Profile", color=discord.Color.random()
        )
    ...
    async def stats(self, ctx, user: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"{user.name}'s Duck and Rocket Stats", color=discord.Color.random()
        )
        embed.add_field(name="Thrust Force", value=f"{data.thrust} Peta Newton")
        embed.add_field(
            name="Fuel Capacity", value=f"{data.fuel} Zeta liters", inline=False
        )
        embed.add_field(name="Max Velocity", value=f"{data.max_velocity} parsec/s")
        embed.add_field(
            name="Duck Intelligence", value=f"{round(data.pilot_intel, 2)}%"
        )

        embed.set_thumbnail(
            url="https://media.discordapp.net/attachments/682157382062964781/919425474382430250/duck-rocket2.png"
        )
        await ctx.reply(embed=embed)
    ...
    async def leaderboard(self, ctx, target: Literal["elo", "money", "xp"] = "elo"):
        res = await self.bot.db.fetchall(
            f"SELECT * FROM user ORDER BY {target} DESC LIMIT 5"
        )
        embed = discord.Embed(
            title=f"🏆 {target.title()} Leaderboard", color=discord.Color.random()
        )
        emoji = (
            "points"
            if target == "elo"
            else self.bot.duckcoin
            if target == "money"
            else "xp"
        )
        for i, r in enumerate(res):
            ...
            embed.add_field(
                ...
                value=f"{getattr(r, target)} {emoji}",
                inline=False,
            )
        await ctx.reply(embed=embed)
    ...
    async def work(self, ctx):
        ...
        earnings = random.randint(5, int(15 * user.level * 0.6))
        ...
        await ctx.reply(
            f"You forced your duck to {w} and earned {earnings} {self.bot.duckcoin}. How cruel."
        )
    ...
    async def claim(self, ctx):
        ...
        await self.bot.db.update_money(ctx.author, earnings)
        await ctx.reply(
            f"You claimed {earnings} {self.bot.duckcoin} from the Elo Leaderboard."
        )
