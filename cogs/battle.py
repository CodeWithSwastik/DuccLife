...
class Battle(commands.Cog):
    ...
    async def challenge(self, ctx, user: discord.Member, bet: int = 10):
        if user == ctx.author:
            ...
            return await ctx.send("You can't challenge bots! Though I'm sure they'd beat you")
        ...
        if p2user_data.money < bet:
            return await ctx.send("They don't have enough money to accept the challenge!")
        ...
        embed = discord.Embed(title=f'Challenge from: {ctx.author.name} [Elo: {p1user_data.elo} | XP: {p1user_data.xp}]', description=f"You'll have to pay {bet} duckcoins if you lose. But if you win you'll get {bet+round(bet/10)} duckcoins!", color=discord.Color.red())
        embed.set_image(url='https://media.discordapp.net/attachments/682157382062964781/919430883977592843/unknown.png')
        ...
        if not view.value:
            render_task.cancel()
            return await ctx.reply(f'{user.name} has declined your challenge. What a coward!')

        embed = discord.Embed(title=f'Let the race begin!', description='Rendering the game...', color=discord.Color.red())
        embed.add_field(name='White Duck', value=ctx.author.name)
        embed.add_field(name='Yellow Duck', value=user.name)
        embed.set_image(url='https://media.discordapp.net/attachments/682157382062964781/919430883977592843/unknown.png')
        embed.set_thumbnail(url='https://cdn.discordapp.com/emojis/822084080211722330.gif?size=160')
        ...
        p1elo, p2elo = new_elo_rating(p1user_data.elo, p2user_data.elo, score[game.winner])
        ...
        await m.edit(
            embed=embed, 
            file=discord.File(image_bytes, filename='ducc.gif')
        )
        if game.winner:
    ...
    async def boss(self, ctx):
        ...
        for i in self.bosses:
            embed.add_field(name=f"{i[0]} | +{i[2]} XP | +{i[3]} {self.bot.duckcoin}", value=str(i[-1]))
    ...
    async def fight(self, ctx, name):
        ...
        if user_data.money < money//10:
            return await ctx.send(f"You don't have enough money to challenge this boss ({money//10} {self.bot.duckcoin})!")
        if user_data.level >= maxlvl:
            return await ctx.send(f"You can't fight this boss since you are overleveled!")
        ...
        embed = discord.Embed(title=f'Boss Battle vs {name}!', description='Rendering the game...', color=discord.Color.red())
        embed.add_field(name='White Duck', value=ctx.author.name)
        ...
        image_bytes = render_task.result()
        embed = discord.Embed(
            title=f'{ctx.author.name} vs {name}!', 
            color=discord.Color.red()
        )
        embed.add_field(name='White Duck', value=ctx.author.name)
        embed.add_field(name='Yellow Duck', value=name)
